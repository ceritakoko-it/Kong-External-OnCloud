...
find "$OUTPUT_DIR" -type f \( -name "*.yaml" -o -name "*.yml" -o -name "*.md" \) -print0 | while IFS= read -r -d '' file; do
  perl -0pe '
    my %repl = (
      ...
      "__PUBLIC_HOST_SECONDARY__" => $ENV{"PUBLIC_HOST_SECONDARY"},
    ...
    s/(__CONTROL_PLANE_NAME__|__PUBLIC_HOST_PRIMARY__|__PUBLIC_HOST_SECONDARY__|__GET_TOKEN_SERVICE_NAME__|__GET_TOKEN_SERVICE_HOST__|__KAOTIM_SERVICE_HOST__|__POKEMON_SERVICE_HOST__|__REDIS_PARTIAL_NAME__|__REDIS_CACHE_PARTIAL_NAME__|__VAULT_CONFIG_STORE_ID__)/$repl{$1}/ge;
  ' "$file" > "${file}.tmp"
  mv "${file}.tmp" "$file"
done
