...
find "$OUTPUT_DIR" -type f \( -name "*.yaml" -o -name "*.yml" -o -name "*.md" \) -print0 | while IFS= read -r -d '' file; do
  perl -0pe '
    ...
    s/(__CONTROL_PLANE_NAME__|__PUBLIC_HOST_PRIMARY__|__GET_TOKEN_SERVICE_NAME__|__GET_TOKEN_SERVICE_HOST__|__ISSUER_URL__|__KAOTIM_SERVICE_HOST__|__STANDARD_CORE_API_USER_CUSTOM_ID__|__STANDARD_GENERAL_SERVICES_USER_CUSTOM_ID__|__REDIS_HOST__|__REDIS_PARTIAL_NAME__|__REDIS_CACHE_PARTIAL_NAME__|__VAULT_CONFIG_STORE_ID__|__OPTIONAL_PUBLIC_HOST_SECONDARY__)/$repl{$1}/ge;
  ' "$file" > "${file}.tmp"
  mv "${file}.tmp" "$file"
done
