...
find "$OUTPUT_DIR" -type f \( -name "*.yaml" -o -name "*.yml" -o -name "*.md" \) -print0 | while IFS= read -r -d '' file; do
  perl -0pe '
    ...
    s/(__CONTROL_PLANE_NAME__|__PUBLIC_HOST_PRIMARY__|__PUBLIC_HOST_SECONDARY__|__GET_TOKEN_SERVICE_NAME__|__REDIS_PARTIAL_NAME__|__REDIS_CACHE_PARTIAL_NAME__)/$repl{$1}/ge;
  ' "$file" > "${file}.tmp"
  mv "${file}.tmp" "$file"
done
